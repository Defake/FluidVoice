...
public nonisolated struct MeetingSynchronizerReplayFixture: Codable, Equatable, Sendable {
...
public nonisolated struct MeetingSynchronizerReplayConfiguration: Codable, Equatable, Sendable {
    ...
    public init(
        ...
        maximumOutputFrameCount: Int = 6000,
    ...
    func makeSynchronizerConfiguration() -> MeetingReferenceSynchronizerConfiguration? {
        ...
        return MeetingReferenceSynchronizerConfiguration(
            ...
            microphoneConverter: .init(algorithmicDelaySeconds: self.microphoneAlgorithmicDelaySeconds),
            referenceConverter: .init(algorithmicDelaySeconds: self.referenceAlgorithmicDelaySeconds),
            referenceToMicrophoneOffsetSeconds: self.referenceToMicrophoneOffsetSeconds,
            referenceClockDriftPPM: self.referenceClockDriftPPM,
            maximumClockDriftPPM: self.maximumClockDriftPPM,
            referenceScope: scope,
            referenceCompleteness: completeness,
            codecPrimingSamples: self.codecPrimingSamples,
            codecRemainderSamples: self.codecRemainderSamples,
            editListOffsetSeconds: self.editListOffsetSeconds,
            maximumInputFrameCount: self.maximumInputFrameCount,
            maximumInputSampleCount: self.maximumInputSampleCount,
            maximumOutputFrameCount: self.maximumOutputFrameCount,
            engineAvailable: self.engineAvailable
        )
    }
}

public nonisolated struct MeetingSynchronizerReplayMicrophoneFrame: Codable, Equatable, Sendable {
    ...
    func makeFrame() -> MeetingMicrophonePCMFrame {
        .init(
...
public nonisolated struct MeetingSynchronizerReplayReferenceFrame: Codable, Equatable, Sendable {
    public let sequenceNumber: Int
    ...
    func makeFrame() -> MeetingReferencePCMFrame {
        .init(
            sequenceNumber: self.sequenceNumber,
...
public nonisolated struct MeetingSynchronizerReplayOutput: Codable, Equatable, Sendable {
    public let schemaVersion: Int
...
public nonisolated struct MeetingSynchronizerReplayDiagnostics: Codable, Equatable, Sendable {
...
public nonisolated enum MeetingReferenceSynchronizerReplayCLI {
    ...
    private static let hardMaximumOutputFrames = 6000
    ...
    public static func run(
        ...
        inputData stdinData: Data,
        output: (Data) -> Void
    ) -> Int32 {
        let gate = arguments.contains(self.argument) || environment[self.environmentKey] == "1"
        guard gate else { output(self.errorData(code: "offlineGateRequired")); return 2 }
        guard let data = inputData(arguments: arguments, stdin: stdinData),
              let fixture = decodeFixture(data), validate(fixture), let config = fixture.configuration.makeSynchronizerConfiguration()
        else {
            output(self.errorData(code: "invalidInput")); return 2
        }
        ...
        guard let canonical = try? canonicalData(fixture) else {
            output(self.errorData(code: "canonicalEncodingFailed")); return 3
        }
        let identity = "synchronizer-replay-v1-" + SHA256.hash(data: canonical).map { String(format: "%02x", $0) }.joined()
        let summary = self.makeOutput(result, identity: identity)
        guard let data = try? encoded(summary) else { output(self.errorData(code: "outputEncodingFailed")); return 3 }
    ...
    private static func inputData(arguments: [String], stdin: Data) -> Data? {
        ...
        return self.readBounded(handle)
    }

    private static func readBounded(_ handle: FileHandle) -> Data? {
        var result = Data()
        while true {
            let remaining = self.maximumInputBytes - result.count
            ...
            result.append(chunk)
            if result.count > self.maximumInputBytes { return nil }
        }
    }
        ...
        guard let configuration = root["configuration"] as? [String: Any],
              strictKeys(
                  ...
                  [
                      "analysisSampleRate",
                      "frameDuration",
                      "microphoneAlgorithmicDelaySeconds",
                      "referenceAlgorithmicDelaySeconds",
                      "referenceToMicrophoneOffsetSeconds",
                      "referenceClockDriftPPM",
                      "maximumClockDriftPPM",
                      "referenceScope",
                      "referenceCompleteness",
                      "codecPrimingSamples",
                      "codecRemainderSamples",
                      "editListOffsetSeconds",
                      "maximumInputFrameCount",
                      "maximumInputSampleCount",
              ...
              microphone.allSatisfy({ strictKeys(
                  $0,
    ...
    private static func validate(_ fixture: MeetingSynchronizerReplayFixture) -> Bool {
        guard fixture.schemaVersion == 1,
              self.finitePositive(fixture.configuration.analysisSampleRate), fixture.configuration.analysisSampleRate <= 384_000,
              self.finitePositive(fixture.configuration.frameDuration), fixture.configuration.frameDuration <= 10,
              self.finite(fixture.configuration.microphoneAlgorithmicDelaySeconds), self.finite(fixture.configuration.referenceAlgorithmicDelaySeconds),
              self.finite(fixture.configuration.referenceToMicrophoneOffsetSeconds), self.finite(fixture.configuration.editListOffsetSeconds),
              self.finite(fixture.configuration.referenceClockDriftPPM), self.finite(fixture.configuration.maximumClockDriftPPM),
              ...
              fixture.configuration.maximumInputFrameCount > 0, fixture.configuration.maximumInputFrameCount <= self.hardMaximumInputFrames,
              fixture.configuration.maximumInputSampleCount > 0, fixture.configuration.maximumInputSampleCount <= self.hardMaximumInputSamples,
              fixture.configuration.maximumOutputFrameCount > 0, fixture.configuration.maximumOutputFrameCount <= self.hardMaximumOutputFrames,
        ...
        guard frameProduct.isFinite, frameSize >= 1, frameSize <= Double(self.hardMaximumOutputSamples),
        ...
        guard !outputOverflow, outputSamples <= self.hardMaximumOutputSamples else { return false }
        var allSamples = 0
        ...
        guard fixture.microphone.allSatisfy(self.validate), fixture.reference.allSatisfy(self.validate) else { return false }
        return true
    }

    private static func validate(_ frame: MeetingSynchronizerReplayMicrophoneFrame) -> Bool {
        !frame.samples.isEmpty && frame.channelCount > 0 && frame.channelCount <= self.hardMaximumChannelCount
            && self.finitePositive(frame.sampleRate) && frame.sampleRate <= self.hardMaximumSourceSampleRate
            && frame.samples.count % frame.channelCount == 0
            && frame.samples.allSatisfy(\.isFinite) && (frame.hostTime.map(self.finite) ?? true)
    }
    ...
    private static func makeOutput(_ result: MeetingSynchronizationResult, identity: String) -> MeetingSynchronizerReplayOutput {
        ...
        for frame in frames {
            for reason in frame.unknownReasons {
                reasons[reason.rawValue, default: 0] += 1
            }
        }
        var epochs = [Int](repeating: 0, count: max(0, (frames.map(\.epochID).max() ?? -1) + 1))
        for frame in frames where frame.epochID >= 0 && frame.epochID < epochs.count {
            epochs[frame.epochID] += 1
        }
        ...
        return .init(
            ...
            captureInvalidSampleCount: frames.reduce(0) { $0 + $1.captureValidMask.filter { !$0 }.count },
            renderValidSampleCount: renderValid,
            renderInvalidSampleCount: frames.reduce(0) { $0 + $1.renderValidMask.filter { !$0 }.count },
            unknownReasonCounts: reasons,
            epochFrameCounts: epochs,
            sessionStartTime: sessions.map(\.0).min(),
            sessionEndTime: sessions.map(\.1).max(),
            microphoneSourceObservedMinimumTime: micTimes.min(),
            microphoneSourceObservedMaximumTime: micTimes.max(),
            referencePTSObservedMinimumTime: refTimes.min(),
            referencePTSObservedMaximumTime: refTimes.max(),
            diagnostics: .init(
                inputFrameCount: result.diagnostics.inputFrameCount,
    ...
    private static func errorData(code: String) -> Data {
        ...
        return (try? self.encoded(ErrorOutput(schemaVersion: 1, errorCode: code)))
