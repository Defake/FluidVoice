...
final class MeetingReferenceSynchronizerReplayCLITests: XCTestCase {
        ...
        self.fixture(
            microphone: (0..<3).map {
                ...
                return .init(
                    ...
                    hostTime: hostTime.map { base in base + Double(index) * 0.01 },
                    sampleRate: 16_000,
                    samples: [Float](repeating: Float(index + 1), count: 160)
                )
            },
            reference: (0..<3).map {
                .init(
                    sequenceNumber: $0,
                    presentationTime: Double($0) * 0.01,
                    sampleRate: 16_000,
    ...
    private func run(_ fixture: MeetingSynchronizerReplayFixture, environment: [String: String] = [:]) throws -> (Int32, Data) {
        var output = Data()
        let status = try MeetingReferenceSynchronizerReplayCLI.run(
            arguments: ["replay", MeetingReferenceSynchronizerReplayCLI.argument],
            environment: environment,
            inputData: self.encoded(fixture),
            output: { output = $0 }
        )
    ...
    func testUnknownMasksGapsEpochsAndSynthTimingAreSummarized() throws {
        let mic = [
            MeetingSynchronizerReplayMicrophoneFrame(
                sequenceNumber: 0,
                sampleTime: 0,
                hostTime: nil,
                sampleRate: 16_000,
    ...
    func testMalformedSchemaAndNonFiniteInputFailClosedWithoutPathLeak() throws {
        var json = try XCTUnwrap(JSONSerialization.jsonObject(with: self.encoded(self.frameFixture())) as? [String: Any])
    ...
    func testResourceBoundsAndOfflineGateAreExplicit() throws {
        var output = Data()
        let noGate = try MeetingReferenceSynchronizerReplayCLI.run(
            arguments: ["replay"], inputData: self.encoded(self.frameFixture()), output: { output = $0 }
        )
    ...
    func testNilHostTimeAndEmptySamplesAreHandledStrictly() throws {
        let nilHost = self.frameFixture(hostTime: nil)
    ...
    func testOutputAllocationBudgetHasAnExactFrameCap() throws {
        let accepted = MeetingSynchronizerReplayConfiguration(maximumOutputFrameCount: 6000)
        let (acceptedStatus, _) = try run(frameFixture(configuration: accepted))
        XCTAssertEqual(acceptedStatus, 0)

        let tooManyFrames = MeetingSynchronizerReplayConfiguration(maximumOutputFrameCount: 6001)
    ...
    func testConfiguredFixtureFamiliesReplayWithoutClassificationClaims() throws {
        ...
        let lowReference = broadbandReference.map { $0 * 0.00_001 }
        ...
        for family in families {
            let mic = (0..<3).map { index in
                ...
                return MeetingSynchronizerReplayMicrophoneFrame(
                    sequenceNumber: index,
                    sampleTime: Int64(index * 160),
                    hostTime: 1 + Double(index) * 0.01,
                    sampleRate: 16_000,
                    samples: slice
                )
            }
            let ref = (0..<3).map { index in
                ...
                return MeetingSynchronizerReplayReferenceFrame(
                    sequenceNumber: index,
                    presentationTime: Double(index) * 0.01,
                    sampleRate: 16_000,
                    samples: slice
                )
            }
            let replay = self.fixture(microphone: mic, reference: ref)
            ...
            let result = synchronizer.synchronize(
                microphone: mic.map {
                    MeetingMicrophonePCMFrame(
                        ...
                        samples: $0.samples,
                        routeIdentifier: $0.routeIdentifier,
