...
@MainActor
final class FloatingOnboardingOverlayTests: XCTestCase {
    func testOnboardingUsesRealFloatingWindowWithoutChangingPreferences() async {
        let manager = NotchOverlayManager.shared
        let owner = UUID()
        let settings = SettingsStore.shared
        let position = settings.overlayPosition
        let selection = settings.dictationPromptSelection(for: .primary)
        let audio = PassthroughSubject<CGFloat, Never>()
        manager.beginOnboardingOverlay(owner: owner)
        defer { manager.endOnboardingOverlay(owner: owner) }
        manager.show(audioLevelPublisher: audio.eraseToAnyPublisher(), mode: .dictation)
        audio.send(0.4)
        try? await Task.sleep(for: .milliseconds(30))
        XCTAssertEqual(NotchContentState.shared.bottomOverlayAudioLevel, 0.4)
        XCTAssertTrue(manager.isBottomOverlayVisible)
        XCTAssertFalse(manager.isOverlayVisible)
        manager.setProcessing(true)
        XCTAssertTrue(NotchContentState.shared.isProcessing)
        XCTAssertTrue(manager.isBottomOverlayVisible)
        XCTAssertFalse(manager.isOverlayVisible)
        let outcome = await manager.hideAndWait()
        XCTAssertEqual(outcome, .hidden)
        audio.send(0.9)
        try? await Task.sleep(for: .milliseconds(30))
        XCTAssertEqual(NotchContentState.shared.bottomOverlayAudioLevel, 0)
        XCTAssertEqual(settings.overlayPosition, position)
        XCTAssertEqual(settings.dictationPromptSelection(for: .primary), selection)
    }

    func testStalePageExitDoesNotReleaseNewFloatingPresentation() async {
        let manager = NotchOverlayManager.shared
        let first = UUID()
        let second = UUID()
        manager.beginOnboardingOverlay(owner: first)
        manager.beginOnboardingOverlay(owner: second)
        defer { manager.endOnboardingOverlay(owner: second) }
        manager.endOnboardingOverlay(owner: first)
        manager.show(audioLevelPublisher: Empty<CGFloat, Never>().eraseToAnyPublisher(), mode: .dictation)
        manager.setProcessing(true)
        XCTAssertTrue(manager.isBottomOverlayVisible)
        XCTAssertFalse(manager.isOverlayVisible)
