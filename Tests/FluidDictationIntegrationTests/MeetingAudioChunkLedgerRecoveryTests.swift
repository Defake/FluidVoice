...
final class MeetingAudioChunkLedgerRecoveryTests: XCTestCase {
    func testReadyPCMAssetMustMatchConfinedBytesAndHash() async throws {
        ...
        let asset = MeetingAudioAsset(
            ...
            presence: .ready,
            relativeFilePath: "tracks/microphone/pcm.caf",
        ...
        let chunk = MeetingAudioChunk(
            id: UUID(),
            sequence: 0,
            relativeFilePath: "legacy.m4a",
            presentationStart: MeetingMediaTime(value: 0, timescale: 1000),
            presentationEnd: MeetingMediaTime(value: 1, timescale: 1000),
            discontinuities: [],
            sha256: hash,
            byteCount: Int64(bytes.count),
        ...
        var session = MeetingSession(
            id: sessionID,
            configuration: MeetingCaptureConfiguration(mode: .inRoom, title: "P0b", microphone: MeetingMicrophoneIdentity(captureDeviceID: "m", displayName: "M")),
    ...
    func testPartialOrEscapingNewAssetIsNeverPromoted() async throws {
        let chunk = MeetingAudioChunk(
            ...
            sha256: "legacy",
            byteCount: 1,
            finalizationState: .writing,
            audioSchemaVersion: 2,
            captureAnalysisAsset: MeetingAudioAsset(
                role: .captureAnalysis,
                encoding: .linearPCMFloat32CAFV1,
                presence: .partial,
                relativeFilePath: "../outside.partial.caf",
                byteCount: 0
            )
        )
    ...
    private func makeDirectory() throws -> URL { let url = FileManager.default.temporaryDirectory
        .appendingPathComponent("FluidVoice-P0b-recovery-\(UUID().uuidString)"); try FileManager.default.createDirectory(
            at: url,
