...
struct OnboardingGlassSurface: ViewModifier {
    ...
    // Painted glass keeps perspective shots and offline renders identical.
    var useNativeGlass = false
    ...
    func body(content: Content) -> some View {
        self.material(content: content)
            .overlay {
                if !self.reduceTransparency && !self.forceOpaque {
    ...
    @ViewBuilder private func material(content: Content) -> some View {
        ...
        } else if #available(macOS 26, *), self.useNativeGlass {
            content.background {
                Color.clear.glassEffect(.regular.tint(FluidBrandColors.blue.opacity(self.selected ? 0.08 : 0.025)), in: self.shape)
            }
        } else {
            content.background(self.shape.fill(Color(red: 0.07, green: 0.095, blue: 0.15)))
        }
    }
}
