...
nonisolated enum MeetingDetectorFeasibilityProbe {
    ...
    static func startIfRequested() {
        ...
        guard self.gate.checkAndSet() else { return }
...
private final nonisolated class ProbeSession: @unchecked Sendable {
    ...
    private var deadlineMs: Int { self.durationSeconds * 1000 }
    ...
    func start() {
        self.controlQueue.async { self.setup() }
    }

    func cancel() {
        self.controlQueue.async { self.finalize(.cancelled) }
    }

    // MARK: - Control queue

    private func setup() {
        guard self.terminalReason == nil else { return }
        guard Self.validateSchema() else {
            self.finalize(.validationFailure)
            return
        }
        ...
        self.outputPath = candidate.path
        do {
            self.fileHandle = try FileHandle(forWritingTo: candidate)
        } catch {
        ...
        guard self.writeLine(startLine) else { return }

        let timer = DispatchSource.makeTimerSource(queue: self.controlQueue)
        timer.schedule(deadline: .now() + .seconds(self.durationSeconds))
        timer.setEventHandler { [self] in
            self.finalize(self.sampleInFlight ? .deadlineExceeded : .completed)
        }
        self.deadlineTimer = timer
        timer.resume()

        self.scheduleTick(after: 0)
    }

    private func scheduleTick(after seconds: Int) {
        self.sampleInFlight = true
        self.samplingQueue.asyncAfter(deadline: .now() + .seconds(seconds)) { [self] in
            let snapshot = self.captureSnapshot()
            self.controlQueue.async { [self] in
                self.sampleInFlight = false
                guard self.terminalReason == nil else { return }
                guard Self.milliseconds(from: self.baseInstant) < self.deadlineMs else {
                    self.finalize(.deadlineExceeded)
                    return
                }
                guard self.writeLine(snapshot) else { return }
                if Self.milliseconds(from: self.baseInstant) + 1000 < self.deadlineMs {
                    self.scheduleTick(after: 1)
                }
            }
        }
    }
    ...
    private func writeLine(_ line: some Codable) -> Bool {
        ...
        let payload: Data
        do {
            payload = try self.encoder.encode(line)
        } catch {
            self.finalize(.encodingFailure)
            return false
        }
        var framed = payload
        framed.append(0x0a)
    ...
    private func finalize(_ reason: TerminalReason) {
        ...
        if let handle = fileHandle {
            let terminal = ProbeTerminalLine(
                schemaVersion: 1,
                event: .terminal,
                monotonicOffsetMs: Self.milliseconds(from: self.baseInstant),
                reason: reason,
        ...
        self.terminalReason = outcome
        let message = "terminal=\(outcome.rawValue) records=\(self.recordCount) output=\(self.outputPath)"
    ...
    private func captureSnapshot() -> ProbeSampleLine {
        ...
        var processCount = 0
        switch self.readAudioObjectIDList(kAudioHardwarePropertyProcessObjectList) {
        case let .ok(ids):
            processListStatus = .ok
            processCount = ids.count
            for id in ids {
                let family = Self.meetingFamily(for: self.readProcessBundleID(id))
                guard family != .other else { continue }
                processes.append(ProcessEntry(
                    family: family,
                    input: self.readProcessRunning(id, selector: kAudioProcessPropertyIsRunningInput),
                    output: self.readProcessRunning(id, selector: kAudioProcessPropertyIsRunningOutput)
                ))
            }
        ...
        switch self.readAudioObjectIDList(kAudioHardwarePropertyDevices) {
        case let .ok(ids):
            deviceListStatus = .ok
            hardwareDeviceCount = ids.count
            for id in ids {
                let stream = self.readInputChannels(id)
                ...
                devices.append(DeviceEntry(
                    token: self.readDeviceUID(id).map(self.saltedToken(for:)),
                    inputChannels: stream.channels,
                    streamStatus: stream.status,
                    transport: self.readTransport(id),
                    running: self.readRunningSomewhere(id)
                ))
            }
        ...
        let window = self.captureWindowList()
    ...
    private func saltedToken(for uid: String) -> String {
        var hasher = SHA256()
        hasher.update(data: Data(self.salt))
    ...
    private func readProcessBundleID(_ objectID: AudioObjectID) -> String? {
        self.readStringProperty(objectID, selector: kAudioProcessPropertyBundleID)
    }
    ...
    private func readDeviceUID(_ objectID: AudioObjectID) -> String? {
        self.readStringProperty(objectID, selector: kAudioDevicePropertyDeviceUID)
    }
