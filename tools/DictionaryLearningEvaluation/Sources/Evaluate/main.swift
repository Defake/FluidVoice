  ...
  static func main() async throws {
    ...
    for language in ["en_us", "es_419", "fr_fr", "de_de", "ru_ru", "pl_pl", "el_gr", "da_dk"] {
      ...
      for file in try FileManager.default.contentsOfDirectory(
        ...
        guard samples.count <= 238080 else {
        ...
        for (index, word) in words.enumerated() {
          ...
          output.append(
            Word(
              clip: clip, language: language, index: index, text: normalized(word.text),
              reference: labels[index], start: word.startTime, end: word.endTime,
              values: embedding.values, frames: embedding.sourceFrameCount))
        }
    ...
    for key in grouped.keys.sorted() {
      ...
      let focalStart = Int((enrollment.start * 16000).rounded(.down))
      let focalEnd = min(source.samples.count, Int((enrollment.end * 16000).rounded(.up)))
      let contextStart = max(0, focalStart - 32000) / 1280 * 1280
      let contextEnd = min(source.samples.count, contextStart + 238080)
      ...
      where clip != enrollment.clip && clip.hasPrefix(enrollment.language) {
        ...
        let matches = PronunciationEmbeddingMatcher.allMatches(
          prototypes: [prototype], in: clips[clip]!.features)[0]
        ...
        trials.append(
          Trial(
            language: enrollment.language, target: enrollment.reference!, known: enrollment.text,
            clip: clip, words: words, hits: hits))
      }
    }
