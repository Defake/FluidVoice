...
@main struct PersonalProof {
  static func main() async throws {
    guard CommandLine.arguments.count == 4 else {
      throw NSError(
        domain: "DictionaryLearningEvaluation", code: 1,
        userInfo: [
    ...
    var allWords: [Token] = []
    for start in stride(from: 0, to: source.count, by: 238080) {
      let samples = Array(source[start..<min(source.count, start + 238080)])
      ...
      let words = WordAudioChunkExtractor.words(from: result.tokenTimings ?? []).map {
        Token(
          text: normalize($0.text), start: $0.startTime + Double(start) / 16000,
          end: $0.endTime + Double(start) / 16000)
      }
    ...
    for target in ["nvidia", "jensen", "huang", "design", "ai"] {
      ...
      let lower = Int((enrollment.start * 16000).rounded(.down))
      let upper = Int((enrollment.end * 16000).rounded(.up))
      let contextStart = max(0, lower - 32000) / 1280 * 1280
      let contextEnd = min(source.count, contextStart + 238080)
      ...
      for window in windows {
        ...
        if enrollment.start >= Double(window.offset) / 16000
          && enrollment.start < Double(window.offset + 238080) / 16000
        {
          continue
        }
        ...
        let matches = PronunciationEmbeddingMatcher.allMatches(
          prototypes: [prototype], in: window.features)[0]
        ...
        for match in matches {
          let start = Double(window.offset) / 16000 + Double(match.frameRange.lowerBound) * 0.08
          let end = Double(window.offset) / 16000 + Double(match.frameRange.upperBound) * 0.08
          ...
          hits.append(
            Hit(
              score: match.score, text: words.map(\.text).joined(separator: " "), start: start,
              end: end, targetOnly: words.count == 1 && words[0].text == target))
        }
      }
      trials.append(
        Trial(
          target: target, enrollmentStart: enrollment.start, positiveOccurrences: positives,
          hits: hits))
    }
